...
<div class="row mb-3">
    <div class="col-md-4">
        ...
        <input type="text" class="form-control @error('previous_passport') is-invalid @enderror"
            name="previous_passport" id="previous_passport"
            value="{{ old('previous_passport') ?? $passenger->passport->previous_passport }}">
    ...
    <div class="col-md-4">
        ...
        <input type="text" class="form-control @error('passport_no') is-invalid @enderror" name="passport_no"
            id="passport_no" value="{{ old('passport_no') ?? $passenger->passport->passport_no }}" required>
        @error('passport_no')
            <span class="text-danger">{{ $message }}</span>
        @enderror
    </div>
    <div class="col-md-4">
        ...
        <select class="form-control @error('passport_type') is-invalid @enderror" name="passport_type"
            ...
            @foreach ($passport_types as $passport_type)
                @php
                    $isSelected =
                        old('passport_type') == $passport_type->value ||
                        $passport_type->value == $passenger->passport->passport_type->value;
                @endphp
                <option value="{{ $passport_type->value }}" @selected($isSelected)>
...
<div class="row mb-3">
    <div class="col-md-4">
        ...
        <input type="date" class="form-control @error('passport_issue_date') is-invalid @enderror"
            ...
            value="{{ old('passport_issue_date') ?? $passenger->passport->passport_issue_date }}" required>
    ...
    <div class="col-md-4">
        ...
        <select class="form-control @error('passport_issue_place') is-invalid @enderror" name="passport_issue_place"
            ...
            @foreach ($districts as $district)
                @php
                    $isSelected =
                        old('passport_issue_place') == $district->id ||
                        $passenger->passport->passport_issue_place == $district->id;
                @endphp
                <option value="{{ $district->id }}" @selected($isSelected)>{{ $district->name }}</option>
            @endforeach
        </select>
    ...
    <div class="col-md-4">
        ...
        <input type="date" class="form-control @error('passport_expire_date') is-invalid @enderror"
            name="passport_expire_date" id="passport_expire_date"
            value="{{ old('passport_expire_date') ?? $passenger->passport->passport_expire_date }}" required>
